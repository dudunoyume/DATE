...
class DAT(object):
    def __init__(self):
        ...
        # 圖的錨定點
        #global image
        self.image = canvas.create_image( 75, 0, anchor="n", image=self.image_file)
        canvas.pack(side="left")
        tk.Label(self.window, text='獅白嘲笑你很忙', font=('標楷體', 16)).pack()
        # ---------------------------------------------------------------------------------------------------------------------------------------------------------------

        # key 入資料位置   ## 1.輸入檔案位置 2.日期欄位 3.輸出分隔式 4.輸出年分民國OR 西元  5.輸出檔案位置
        tk.Label(self.window, text="輸入檔案位置 (須為excel or csv 檔): ",font=('標楷體', 12)).place(x=160, y=30)

        tk.Label(self.window, text="日期欄位名: ",font=('標楷體', 12)).place(x=160, y=100)

        tk.Label(self.window, text="日期分隔符號", font=('標楷體', 12)).place(x=160, y=170)  # 下拉式

        tk.Label(self.window, text="年份格式", font=('標楷體', 12)).place(x=160, y=240)  # check box

        tk.Label(self.window, text="輸出檔案位置(須為excel or csv 檔): ",font=('標楷體', 12)).place(x=160, y=310)
        ...
        self.path1.set('C:/')
        self.entry_path1 = tk.Entry(self.window, textvariable=self.path1, font=('Arial', 12), width=30)
        self.entry_path1.place(x=160, y=60)
        # path1的按鈕，引用path1_button函數
        self.path1_button = tk.Button(self.window, text="輸入檔案", command=self.path1_button)
        self.path1_button.place(x=430, y=60)

        # 日期欄位名文字框-------------------------------------------------------------------------------------------------------------------------------------------------
        self.datecol = tk.StringVar()
        self.datecol.set('日期')
        self.entry_datecol = tk.Entry(self.window, textvariable=self.datecol, font=('標楷體', 12), width=30)
        ...
        rdioOne = tk.Radiobutton(self.window, text='西元年', variable=self.yearcategorize, value="西元年", font=('標楷體', 12))
        rdioOne.place(x=160, y=270)
        rdioOne.select()  # 預設勾選

        rdioTwo = tk.Radiobutton(self.window, text='民國年', variable=self.yearcategorize, value="民國年", font=('標楷體', 12))
        ...
        self.path2.set('C:/')
        self.entry_path2 = tk.Entry(self.window, textvariable=self.path2, font=('Arial', 12), width=30)
        self.entry_path2.place(x=160, y=340)
        # path2的按鈕，引用path2_button函數
        self.path2_button = tk.Button(self.window, text="輸出位址", command=self.path2_button)
        self.path2_button.place(x=430, y=340)

        # 開始跑日期區分------------------------------------------------------------------------------------------------------------------------------------------------

        self.start = tk.Button(self.window, text="開始輸出", command=self.start_button, font=('標楷體', 12), width=30)
    ...
    def path1_button(self):
        # 打開檔案選擇介面
        self.file_path1 = filedialog.askopenfilename(filetypes=[("Excel", "*.xlsx"), ("Excel", "*.xls"),("CSV UTF8", "*.csv")])
        # 更變
    ...
    def path2_button(self):
        ...
        self.file_path2 = filedialog.asksaveasfilename(filetypes=[("Excel ONLY", "*.xlsx"), ("Excel ONLY", "*.xls"),("CSV UTF8", "*.csv")], defaultextension = platform == ".xlsx" and ".csv")
